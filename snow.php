...
if (isset($_GET['page']) && $_GET['page'] == 'wp-FallingSnow/snow.php'){
	wp_enqueue_script('jquery');	
	wp_register_script('my-upload', plugins_url("wp-FallingSnow/script.js"), array('jquery','media-upload','thickbox'));
	wp_enqueue_script('my-upload');
	wp_enqueue_style('thickbox');
	wp_register_style('myStyleSheets', plugins_url("wp-FallingSnow/style.css"));
    wp_enqueue_style( 'myStyleSheets');
}
// create html tags and flash tags
function wp_snow_createflashcode($name,$id,$heightIn,$widthIn){
    $rand = rand(5, 99); 
	$flashtag .= '<div style = "float:left; width:'.$widthIn.'px; height:'.$heightIn.'px; margin:5px; "><script type="text/javascript" src="'.plugins_url("wp-FallingSnow/swfobject/swfobject.js").'" charset="utf-8"></script><script type="text/javascript" src="'.plugins_url("wp-FallingSnow/swfobject/swfaddress.js").'" charset="utf-8"></script><script type="text/javascript">
								var flashvars = {queryPath:"'.plugins_url('wp-FallingSnow/query.php?id='.$rand ).'",BannerID:"'.(string)$id.'"};   
								var params = {};
								var attributes = {};
								...
								swfobject.embedSWF("'.plugins_url("wp-FallingSnow/fallingSnow.swf?id=".$rand ).'", "'.$name.'-'.$id.'", "'.$widthIn.'px", "'.$heightIn.'px", "9.0.0", "'.plugins_url("wp-FallingSnow/swfobject/expressInstall.swf").'", flashvars, params, attributes);</script><div id="'.$name.'-'.$id.'"></div></div>';
	return $flashtag;
}
...
function wp_snow_options() {	
	...
	Short Code : <br>You can use the plugin numerous times. Just be sure to always enter a different ID in the shortcode. If you need custom image use following shortcode:<br><b><font size="2px">[snow id=1 custom=1 width=300 height=300 upload_image=url number_snow=50 rotationtype=2D]</font></b><br>
				 <font size="0.5px">If you use 2 or more shortcodes on the page or post id numbers must have different values!</font><br><br>
				 If you wish to use the image that you set in settings, use:<br><font size="2px"><b>[snow custom=0]</b></font><br><br>
	...
	echo "<div class=\"wrap\"><h2>Wordpress Flash falling Snow Animation</h2>";
	echo '<table class="form-table">';
	// width
	echo '<tr valign="top"><th scope="row">Width of image used</th>';
	echo '<td><input type="text" name="width" value="'.$row['width'].'" size="5"></input><br />Width of image used in pixels (200 or more is recommended)</td></tr>';
	// height
	echo '<tr valign="top"><th scope="row">Height of image used</th>';
	echo '<td><input type="text" name="height" value="'.$row['height'].'" size="5"></input><br />Height of image used in pixels (300 or more is recommended)</td></tr>';
	//image
	echo '<tr valign="top"><th scope="row">Path do Your Image</th>';
	echo '<td><label for="upload_image">
		  <input id="upload_image" type="text" size="36" name="upload_image" value="'.$row['url'].'" />
		  <input id="upload_image_button" type="button" value="Upload Image" />
		  <br />Enter an URL or upload an image for background.
		  </label></td>
		  </tr>';
	// number_snow
	echo '<tr valign="top"><th scope="row">Number of snow flakes</th>';
	echo '<td><input type="text" name="number_snow" value="'.$row['number_snow'].'" size="5"></input><br />Number of snow flakes used in animation.</td></tr>';			  
	// transition
