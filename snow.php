...
function jal_installsnow() {
   ...
   $result = mysql_query("SELECT count(id) FROM ".$table_snow." where id = 1 ");
   $row = mysql_fetch_array($result);
   if($row['count(id)'] == 0){
   $wpdb->insert( $table_snow , array( 'url' => '', 'width' => '500', 'height' => '300', 'number_snow' => '50' , 
										   'rotationType' => '2D', 'path' => '', 'id_snow' => 0));
	}
	...
	if($custom == 0)
		...
		if($row['count(id)'] == 0){
			$wpdb->insert( $table_snow , array( 'url' => $upload_image, 'width' => $width, 'height' => $height, 'number_snow' => $number_snow , 
													'rotationType' => $rotationtype, 'id_snow'  => $id, 'path' => $path));
		}
		else{
			mysql_query("UPDATE ".$table_snow." SET url = '".$upload_image."' , width = '".$width."', height = '".$height."',  
															number_snow = '".$number_snow."', rotationType = '".$rotationtype."', path = '".$path."' WHERE id_snow = ".$id."") or die(mysql_error());
		}
...
function wp_snow_options() {	
	...
	if (!empty($_POST["snow_submit"]) ) {
		...
		$newoptions['path'] = strip_tags(stripslashes($_POST["path"]));
	...
	if ( $options != $newoptions ) {
		...
		mysql_query("UPDATE ".$table_snow." SET url = '".$newoptions['upload_image']."' , width = '".$newoptions['width']."', height = '".$newoptions['height']."',  
															number_snow = '".$newoptions['number_snow']."', rotationType = '".$newoptions['rotationType']."', path = '".$newoptions['path']."' WHERE id = 1");
	}
	global $wpdb;
	$table_snow = $wpdb->prefix . "snow";
	...
	Short Code : <br>If you need custom image use:<br><b><font size="2px">[snow id=1 custom=1 width=300 height=300 upload_image=url number_snow=50 rotationtype=2D path=http://premiumcoding.com]</font></b><br>
    ...
    Visit our support site for more help <a href="http://premiumcoding.com/wordpress-falling-snow-plugin/">PremiumCoding WordPress falling Snow</a>.<br>
	...
	//image
	echo '<tr valign="top"><th scope="row">Path to Image</th>';
	...
	// path
	echo '<tr valign="top"><th scope="row">Url</th>';
	echo '<td><input type="text" name="path" value="'.$row['path'].'" size="36"></input><br />Url</td></tr>';		  
	// number_snow
